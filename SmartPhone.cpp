...
SmartPhone::SmartPhone(const char versOs[15], const char conex[15], const char nume[20], int spatiuStocare, int memorie, int capacitatebaterie, const Ecran &e)
        : Gadget(versOs, conex), _spatiuStocare(spatiuStocare), _memorie(memorie), _capacitatebaterie(capacitatebaterie), _e(e) {
    strcpy(_nume,nume);
}

SmartPhone::SmartPhone(const SmartPhone &ob) : Gadget(ob.getOs(), ob.getConexiune()),
                                               _spatiuStocare(ob.getSpatiuStocare()), _memorie(ob.getMemorie()),
                                               _capacitatebaterie(ob.getCapacitatebaterie()){
